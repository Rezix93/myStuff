...
public class TaskFlowProvider extends AbstractTmfTraceDataProvider implements ITimeGraphDataProvider<@NonNull ITimeGraphEntryModel>, IOutputStyleProvider {

    /**
     * Provider unique ID.
     */
    public static final String ID = "org.eclipse.tracecompass.incubator.internal.spark_test1.core.analysis.sparktaskflow"; //$NON-NLS-1$
    ...
    private SparkAnalysisModule fModule;
    private static final int MARKER_SIZE = 500;
    //private RedisNetArrowProvider fArrowProvider;
    private Map <Integer, Long> tidToEntryId = new HashMap();
     ...
     */
    public TaskFlowProvider(ITmfTrace trace, SparkAnalysisModule module) {
        super(trace);
        fModule = module;

        //fArrowProvider=new RedisNetArrowProvider(trace);
    }
    ...
    public static @Nullable ITmfTreeDataProvider<? extends ITmfTreeDataModel> create(ITmfTrace trace) {
        SparkAnalysisModule module = TmfTraceUtils.getAnalysisModuleOfClass(trace, SparkAnalysisModule.class, SparkAnalysisModule.ID);
        return module != null ? new TaskFlowProvider(trace, module) : null;
    }
    ...
    public TmfModelResponse<TmfTreeModel<@NonNull ITimeGraphEntryModel>> fetchTree(Map<String, Object> fetchParameters, @Nullable IProgressMonitor monitor) {
        ...
        //List list = ss.getQuarks("Kernel Thread Task","*");

        for (Integer quark : ss.getQuarks("Kernel Thread Task","*")) {
                // get attribute name (thread id for the threads quark) and associate id to the tid.
                Long id = fIDToDisplayQuark.inverse().computeIfAbsent(quark, q -> sfAtomicId.getAndIncrement());
                tidToEntryId.put(Integer.parseInt(ss.getAttributeName(quark)), id);
    ...
    private static TimeGraphState getStateFromInterval(ITmfStateInterval statusInterval, long currentEndTime) {
        ...
        String styleName = "style"+Math.abs(o.hashCode()%4); //$NON-NLS-1$
