...
check konam age enable all konam chi mishe ? 

LD_PRELOAD="/usr/lib/x86_64-linux-gnu/liblttng-ust-fork.so:/usr/lib/x86_64-linux-gnu/jni/liblttng-ust-log4j-jni.so" exec /usr/lib/jvm/java-11-openjdk-amd64/bin/java -Djava.library.path=.:/usr/local/lib -cp "/opt/spark/conf/:/opt/spark/assembly/target/scala-2.12/jars/*:/etc/hadoop:/opt/spark/core/target/jars/lttng-agent/*" -Xmx1g org.apache.spark.deploy.master.Master --host Rezghool --port 7077 --webui-port 8080 -p 7077
...
```
error: 
#14 10.96 environment variable, e.g. ./configure CLASSPATH="/path/to/log4j.jar"
...
find / -name lttng-ust-agent-log4j.jar && /
find / -name lttng-ust-agent-common.jar && /
find / -name liblttng-ust-log4j-jni.so
